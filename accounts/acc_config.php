...
// Disable the security code requirement for logged in users when posting ads.
// Default 0, 1 to enable.

$no_captcha_logged_in = 1;


// Email used to send automated emails from your site

$main_acc_email = 'your@email.here'; 
...
// IMPORTANT: Don't forgot the first dot before the domain name. Like .google.com

$cookie_domain = '.sitedomain.com';
...
$acc_panel_link = ($sef_urls) ? 'myaccount' : 'index.php?view=userpanel';
$acc_login_link = ($sef_urls) ? 'signin' : 'index.php?view=login';
$acc_signup_link = ($sef_urls) ? 'signup' : 'index.php?view=signup';
...
if ( $main_acc_email == 'your_email@your_site.com' && $cookie_domain == '.your_site.com' )
{
	// This message appears if you do not change your account email or cookie domain.
	echo "The site admin hasn't configured accounts yet.";
}
